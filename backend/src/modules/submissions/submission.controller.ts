...
export const getSubmissionsByAssignment = asyncWrapper(async (req: AuthRequest, res: Response) => {
...
export const gradeSubmission = asyncWrapper(async (req: AuthRequest, res: Response) => {
    const data = gradeSubmissionSchema.parse(req.body);
    const submission = await submissionService.gradeSubmission(req.params.id as string, data, req.user!.id);
