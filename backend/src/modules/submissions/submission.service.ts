...
export class SubmissionService {
    ...
    async gradeSubmission(submissionId: string, data: z.infer<typeof gradeSubmissionSchema>, instructorId: string) {
        const submission = await Submission.findById(submissionId);
        if (!submission) throw new ApiError(404, 'Submission not found');

        // Verify the grader is the instructor of this assignment's course
        ...
        // Validate grade does not exceed the assignment's maxScore
        if (data.grade > assignment.maxScore) {
            throw new ApiError(400, `Grade cannot exceed assignment max score of ${assignment.maxScore}`);
        }
