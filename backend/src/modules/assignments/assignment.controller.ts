...
export const getAssignmentsByCourse = asyncWrapper(async (req: Request, res: Response) => {
    const assignments = await assignmentService.getAssignmentsByCourse(req.params.courseId as string);
...
export const updateAssignment = asyncWrapper(async (req: AuthRequest, res: Response) => {
    const assignment = await assignmentService.updateAssignment(req.params.id as string, req.user!.id, req.body);
