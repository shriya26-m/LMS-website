...
export const updateCourse = asyncWrapper(async (req: AuthRequest, res: Response) => {
    ...
    const course = await courseService.updateCourse(req.params.id as string, req.user!.id, data, req.user!.role);
...
export const deleteCourse = asyncWrapper(async (req: AuthRequest, res: Response) => {
    await courseService.deleteCourse(req.params.id as string, req.user!.id, req.user!.role);
    res.json(new ApiResponse(200, 'Course deleted'));
});

export const publishCourse = asyncWrapper(async (req: AuthRequest, res: Response) => {
    const course = await courseService.publishCourse(req.params.id as string, req.user!.id);
...
export const enrollStudent = asyncWrapper(async (req: AuthRequest, res: Response) => {
    const course = await courseService.enrollStudent(req.params.id as string, req.user!.id);
...
export const getEnrolledStudents = asyncWrapper(async (req: Request, res: Response) => {
    const students = await courseService.getEnrolledStudents(req.params.id as string);
...
export const getAdminAllCourses = asyncWrapper(async (req: Request, res: Response) => {
    const { page = '1', limit = '10', search, category } = req.query as Record<string, string>;
    const result = await courseService.getAdminAllCourses(Number(page), Number(limit), search, category);
    res.json(new ApiResponse(200, 'All courses (admin)', result));
});

export const getMonthlyEnrollments = asyncWrapper(async (_req: Request, res: Response) => {
    const data = await courseService.getMonthlyEnrollments();
    res.json(new ApiResponse(200, 'Monthly enrollments', data));
});
