...
export const coursesService = {
    ...
    // Admin: fetch ALL courses (published + draft)
    getAllAdmin: (page = 1, limit = 10, search?: string, category?: string) =>
        api.get<ApiResponse<PaginatedResponse<Course>>>('/courses/admin-all', { params: { page, limit, search, category } }),

    // Admin: monthly enrollment analytics
    getMonthlyEnrollments: () =>
        api.get<ApiResponse<{ month: string; enrollments: number }[]>>('/courses/monthly-enrollments'),
};
